Shader "My/ovo/instancing/ovo_pbr_transparent_z"
{
    Properties
    {
    ...
    SubShader
    {
		...
		/*
        Pass // 前向渲染 Additional Pass
        {
            Tags { "LightMode"="ForwardAdd" }
            //设置 base Pass 和 Additional pass 的混合计算
            Blend One One            
            
            CGPROGRAM

            //如果需要此物体接收点光源和聚光灯光源的阴影 则需要使用 multi_compile_fwdadd_fullshadows
            //值得注意的是，如果没使用此命令，Unity不会为这些光源计算阴影，即使在 Additional Pass 中计算了阴影也不会生效
            #pragma multi_compile_fwdadd_fullshadows
            //如果不需要此物体接收点光源和聚光灯光源的阴影，则只需使用 multi_compile_fwdadd 命令
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                //_MainTex 贴图采样，并于颜色参数混合
                fixed4 albedo = tex2D(_MainTex, i.uv) * _Color;
                // 环境光已经在 Base Pass 种计算过，所以在 Additional Pass 无需再次计算环境光
                //fixed3 ambient = UNITY_LIGHTMODEL_AMBIENT.rgb * albedo.rgb;
                ...
                // /*Half-Lambert模型*/
                // fixed3 diffuse = albedo.rgb * _LightColor0.rgb * (dot(worldNormal, worldLightDir)* 0.5 + 0.5);
